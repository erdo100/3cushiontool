    ...
    for set_item in data.get("Match", {}).get("Sets", []):
        ...
        for shot in set_item.get("Entries", []):
            ...
            for dataset in path_tracking.get("DataSets", []):
                ...
                if not coords:
                    print(f"Shot ID {shot_id}: No coordinates found for ball '{ball_index}'.")
...
def delete_outside_table(all_shots):
    ...
    for shot in all_shots:
        for ball in shot['balls'].values():
            index = np.where((ball['x'] < ball_radius) | (ball['x'] > tablesize[0] - ball_radius) | (ball['y'] < ball_radius) | (ball['y'] > tablesize[1] - ball_radius))
        ...
        for i in range(1, len(balls)):
            for j in range(i):
                ...
                if distance < 0:
                    ...
                    if centerdistance == 0:
                        print(f"Shot ID {shot['shotID']}: Balls are at the same position. Deleting shot.")
...
def extract_b1_b2_b3(all_shots):
    for shot in all_shots:
        ...
        if any(len(balls[color]['t']) < 2 for color in ball_colors):
            print(f"Shot ID {shot['shotID']}: Not enough data points to identify b1, b2, b3.")
            continue
        ...
        if len(b1['t']) >= 3:
            ...
            if len(tb2i2) == 0 and len(tb3i2) == 0 and len(tb2i3) == 0 and len(tb3i3) == 0:
            ...
            if (len(tb2i2) > 0 or len(tb2i3) > 0) and (len(tb3i2) == 0 and len(tb3i3) == 0):
                # B1 and B2 moved
            ...
            if (len(tb2i2) == 0 and len(tb2i3) == 0) and (len(tb3i2) > 0 or len(tb3i3) > 0):
                # B1 and B3 moved
