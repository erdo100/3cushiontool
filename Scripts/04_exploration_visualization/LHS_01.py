...
def plot_3d(xi_name, cut_value, ax):  # Pass the axes object
    ...
    if not samples_df_1.empty:
        ...
        ax.scatter(samples_df_1['a'], samples_df_1['b'], samples_df_1['cut'], c='red', marker='x', label='Point 1', alpha=0.5)
        ...
        if xi_name == 'cut':
            ...
            y_range = np.linspace(samples_df_1['b'].min(), samples_df_1['b'].max(), 100)
            z_range = np.linspace(samples_df_1['cut'].min(), samples_df_1['cut'].max(), 100)
